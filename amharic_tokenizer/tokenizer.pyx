...
cdef class AmharicTokenizer:
    ...
    def __init__(self, int num_merges=50000):
        self._vocabulary = {}
    ...
    cpdef int train(self, str amharic_corpus, bint verbose=False, int log_every=1000):
        ...
        for i in range(self._num_merges):
            if verbose and (i + 1) % log_every == 0:
                print(f"Merge {i + 1}/{self._num_merges} completed.")
            if not pair_counts:
                break
    ...
    cpdef List[int] encode(self, str text):
        ...
        return [self._token_to_id.get(token, -1) for token in tokens]
    ...
    def load(cls, file_path):
        with open(file_path, 'r', encoding='utf-8') as f:
            state = json.load(f)
        tokenizer = cls(num_merges=state.get('num_merges', 50000))
