"""Command-line interface for the Amharic Tokenizer."""
...
def main():
    """Entry point for the Amharic tokenizer command-line interface."""
    parser = argparse.ArgumentParser(
        prog="amh-tokenizer",
        description="Amharic BPE-like tokenizer utilities",
    )
    ...
    p_train.add_argument(
        "output_prefix", help="Output path prefix for saved model")
    p_train.add_argument(
        "--num-merges",
        type=int,
        default=50000,
    ...
    p_train.add_argument(
        ...
        type=int,
        default=1000,
        help="Print status every N merges when verbose",
    )
    ...
    if args.cmd == "train":
        ...
        print(
            f"[AMH-Tokenizer] Saved model to "
            f"{args.output_prefix}.json with {merges} merges"
        )
